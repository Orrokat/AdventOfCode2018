...
function parseAllLogs (logArray) {
  var parsedArray = []
  for (var logIndex = 0; logIndex < logArray.length; logIndex++) {
...
function extractRelevantData (extractedArray) {
  ...
  var sleep = 0
  var wakes = 0
  for (var logIndex = 0; logIndex < extractedArray.length; logIndex++) {
    ...
    if (extractedArray[logIndex][5] === 'wakes') {
      ...
      guardIncidentArray.push({ guard: guard,
        ...
        sleep: sleep,
        wakes: wakes })
    } else { console.log('invalid data ******* ' + extractedArray[logIndex]) }
  }
...
function sumGuardByIncident (incidentObjects) {
  ...
  var currentDate = ''
  var totalDays = 0
  for (var incidentIndex = 0; incidentIndex < incidentObjects.length; incidentIndex++) {
    ...
    for (var minIndex = incidentObjects[incidentIndex].sleep; minIndex < incidentObjects[incidentIndex].wakes; minIndex++) {
      minuteMap[minIndex]++
    }
  }
...
function puzzleSolutionStrategy1 (summaryObjects) {
  ...
  var timesSlept = 0
  for (var guardIndex = 0; guardIndex < summaryObjects.length; guardIndex++) {
...
function solveThePuzzleDay4a (arrayOfLogs) {
  var parsedArray = parseAllLogs(arrayOfLogs.sort())
  var guardIncidentArray = extractRelevantData(parsedArray)
  var guardSumArray = sumGuardByIncident(sortIncidentByGuard(guardIncidentArray))
  return puzzleSolutionStrategy1(guardSumArray)
}
function solveThePuzzleDay4b (arrayOfLogs) {
  var parsedArray = parseAllLogs(arrayOfLogs.sort())
  var guardIncidentArray = extractRelevantData(parsedArray)
  var guardSumArray = sumGuardByIncident(sortIncidentByGuard(guardIncidentArray))
  return puzzleSolutionStrategy2(guardSumArray)
}
